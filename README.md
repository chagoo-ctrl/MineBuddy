...
---
## 🧪 测试功能 v0.0.1-test
通用物品收集测试，支持任意数量任意方块，带简单寻路避障，用于验证原子动作库功能正常。
### 方法命名规范
所有测试方法统一格式：`testGet + 数量 + 物品ID`，例如：
- `testGet10OakLog()`：收集10个橡木
- `testGet16Sand()`：收集16个沙子
- `testGet121Dirt()`：收集121个泥土
- `testGet3IronOre()`：收集3个铁矿石
通用入口：`testGet(int count, String itemId)`，支持任意数量和任意方块ID关键词模糊匹配。
### 测试命令
进入游戏后按T打开聊天框，输入命令：
| 命令 | 说明 |
|------|------|
| `/testget <数量> <物品ID>` | 自动收集指定数量的对应方块，支持模糊匹配 |
| `/testget stop` | 停止当前收集任务 |
| `/testget` | 显示命令帮助和示例 |
#### 命令示例：
| 命令 | 功能 |
|------|------|
| `/testget 10 log` | 收集10个任意原木（橡木/白桦/云杉等） |
| `/testget 10 oak_log` | 只收集10个橡木原木 |
| `/testget 20 stone` | 收集20个石头 |
| `/testget 16 sand` | 收集16个沙子 |
| `/testget 121 dirt` | 收集121个泥土 |
| `/testget 3 iron_ore` | 收集3个铁矿石 |
### 测试逻辑
简单状态机+简单寻路实现：
1. **找目标**：从感知层的可见方块中找24格内最近的匹配方块，没找到就自动转头找，转5秒没找到就往前走探索
2. **移动寻路**：自动看向目标，向前走，距离>8格自动疾跑；撞墙超过1秒自动转15度绕开；卡住超过1.5秒自动跳+后退脱困；普通障碍自动跳跃；到4.5格内停下
...
- 进入世界后，空闲时聊天栏每秒显示绿色运行统计：
  ...
  ```
- 输入`/testget`查看命令帮助，例如`/testget 10 log`开始收集10个木头
### 构建mod jar
```bash
./gradlew build
```
...
- ✅ 超时保护、状态查询功能
#### 0.0.1-test 通用物品收集测试
- ✅ 废弃旧的固定挖木头测试，实现通用物品收集逻辑
- ✅ 支持任意数量（1~64）任意方块，ID模糊匹配
- ✅ 方法名统一为testGet+数量+物品ID格式，如testGet10OakLog
- ✅ 命令改为/testget <数量> <物品ID>，支持参数
- ✅ 升级简单寻路避障：撞墙自动转向、卡住自动跳+后退、自动探索找目标
- ✅ 找目标范围扩大到24格，找不到自动转头+前进探索
- ✅ 10秒挖掘超时保护，挖不动自动换目标
- ✅ 实时进度提示，收集完成自动停止
---
## 📄 开源协议
