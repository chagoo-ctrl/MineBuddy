...
public class MineBuddyClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        System.out.println("[MineBuddy] 命令注册完成，输入 /testget 或 /killsheep 开始测试");
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            KillSheepTest.getInstance().tick();

            tickCounter++;
            // 每秒（20tick）输出一次运行统计，测试运行时不输出避免刷屏
            boolean isTesting = GatherTest.getInstance().isRunning() || KillSheepTest.getInstance().isRunning();
            if (tickCounter % 20 == 0 && !isTesting) {
        ...
        System.out.println("[MineBuddy] 测试命令：");
        System.out.println("[MineBuddy]   /testget <数量> <物品ID> - 自动收集方块，例如 /testget 10 log");
        System.out.println("[MineBuddy]   /killsheep [数量] - 自动杀羊，默认1只，例如 /killsheep 3");
    }
}
